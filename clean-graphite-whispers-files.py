		...
		for filename in filenames:
			if filename.endswith('.wsp'):
				...
				if file_age > selected_days:
					...
					if args.log:
						logging.info(f"{file_path} (age: {file_age/86400:.1f} days, size: {file_size/1048576:.2f} MB, last update: {formatted_time})")

					if args.show:
						print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {file_path} (age: {file_age/86400:.1f} days, last update: {formatted_time})")

					if not args.dry_run:
						print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Deleted {file_path}!")
						logging.info(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Deleted {file_path}!")

	if args.show:
		print("\n-----------------------------------------------------------------------------------------------------------------------------------------")
	print(f"Total Deleted whisper files {counter}, Total deleted size: {total_del_size/(1024*1024):.2f} MB, Total size of all whisper files: {total_size/(1024*1024*1024):.6f} GB, Ratio: {float(float(total_del_size)/float(total_size))*100:.2f}% deleted")
